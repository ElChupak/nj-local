...
const watchFiles = () => {
  ...
  watch('./src/components/scss/**/*.scss', styles).on('change', browserSync.reload);
  watch('./src/js/**/*.js', scripts).on('change', browserSync.reload);
  watch('./src/partials/**/*.html', htmlInclude).on('change', browserSync.reload);
...
const cache = () => {
  return src('app/**/*.{css,js,svg,png,jpg,jpeg,ttf}', {
    base: 'app'})
    .pipe(rev())
    .pipe(revDel())
		.pipe(dest('app'))
...
exports.default = series(clean, htmlInclude, scripts, styles, resources, svgSprites, images, watchFiles);
